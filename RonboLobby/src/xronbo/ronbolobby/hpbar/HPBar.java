...
public class HPBar implements Listener {
	...
	private static RonboLobby plugin;
	
	public HPBar(RonboLobby p) {
		plugin = p;
	}
	...
	public static void setMessage(final Player player, String message, int seconds) {
		...
		timers.put(player.getName(), plugin.getServer().getScheduler().runTaskTimer(plugin, new BukkitRunnable() {
